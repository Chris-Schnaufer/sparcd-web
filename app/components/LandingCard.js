      ...
      <CardActions>
        { actionsIsArray ? 
          curAction.map(function(obj, idx) {
            return <Button size="small" onClick={obj.cb} key={obj.title} sx={{'flex':'1'}}  disabled={obj.disabled}>{obj.title}</Button>;
          })
          : <Button size="small" onClick={curAction.cb} disabled={curAction.disabled}>{curAction.title}</Button>
        }
      </CardActions>
    </Card>
  );
